...
export async function POST(req: NextRequest) {
  ...
  const { data: prof, error } = await sb
    ...
    .select()
    .maybeSingle()
  if (error || !prof) return NextResponse.json({ ok: false, error: 'UPSERT_FAIL' }, { status: 500 })
  ...
  const { data: prof2, error: updErr } = await sb
    ...
    .select()
    .maybeSingle()
  if (updErr || !prof2) return NextResponse.json({ ok: false, error: 'TOKEN_FAIL' }, { status: 500 })
