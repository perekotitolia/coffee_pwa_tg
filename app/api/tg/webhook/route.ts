  ...
  try {
    ...
    if (!text.startsWith("/")) {
      ...
      if (st?.step === "ask_name") {
        ...
        await sendMessage(
          chatId.toString(),
          "Дякую! Тепер надішліть дату народження у форматі YYYY-MM-DD (необов’язково).",
        );
      ...
      if (st?.step === "ask_dob") {
        ...
        if (!dob) {
          await sendMessage(
            chatId.toString(),
            "Будь ласка, використайте формат YYYY-MM-DD, напр.: 1995-07-28",
          );
          return NextResponse.json({ ok: true });
        }
        ...
        await sendMessage(
          chatId.toString(),
          "Чудово! Дані збережено. /qr — ваш QR, /balance — баланс. Якщо не хочете отримувати розсилки — /optout.",
        );
        return NextResponse.json({ ok: true });
      }
    }
    ...
    if (text.startsWith("/start")) {
      ...
      if (param.startsWith("link-")) {
        ...
        if (prof && !error) {
          await sb
            .from("profiles")
            .update({ tg_id: chatId, link_token: null })
            .eq("id", prof.id);
          await sendMessage(
            chatId.toString(),
            "Телеграм успішно привʼязано ✅\nЯк до вас звертатися? Напишіть ім’я.",
          );
        ...
        await sendMessage(
          chatId.toString(),
          "Вітаю! Надішліть /balance щоб перевірити баланс. Ваш QR: команда /qr",
        );
      }
      ...
      if (!prof?.device_id) {
        await sendMessage(
          chatId.toString(),
          "Спочатку привʼяжіть додаток кнопкою у розділі " + site + "/me.",
        );
      } else {
      ...
      await sendMessage(
        chatId.toString(),
        "Ви відписалися від маркетингових повідомлень. /optin — щоб знову підписатися.",
      );
    } else if (text === "/optin") {
      await sb
        .from("profiles")
      ...
      await sendMessage(
        chatId.toString(),
        "Підписка на повідомлення увімкнена. Дякуємо!",
      );
    } else {
      await sendMessage(
        chatId.toString(),
        "Сайт: ${site}, Команди: /start, /balance, /qr, /optin, /optout",
      );
    }
