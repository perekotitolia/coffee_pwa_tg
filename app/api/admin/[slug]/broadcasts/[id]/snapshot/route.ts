...
export async function POST(req: Request, ctx: any) {
  ...
  if (typeof f.min_points === "number")
    q = q.or(`points.is.null,points.gte.${f.min_points}`);
  if (f.last_active_days) {
    const since = new Date(
      Date.now() - f.last_active_days * 24 * 3600 * 1000,
    ).toISOString();
    q = q.gte("last_activity_at", since);
  }
  if (Array.isArray(f.shop_ids) && f.shop_ids.length)
    q = q.in("last_shop_id", f.shop_ids);
